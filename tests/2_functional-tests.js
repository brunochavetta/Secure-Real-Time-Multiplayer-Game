  ...
  suite('Headers test', () => {
    ...
    test("Nothing from the website is cached in the client.", done => {
      chai.request(server)
        .get('/')
        .end((err, res) => {
          console.log(res.headers);  // Verifica los encabezados devueltos
          assert.deepStrictEqual(res.headers['surrogate-control'], 'no-store');
          assert.deepStrictEqual(res.headers['cache-control'], 'no-store, no-cache, must-revalidate, proxy-revalidate');
          assert.deepStrictEqual(res.headers['pragma'], 'no-cache');
          assert.deepStrictEqual(res.headers['expires'], '0');
          done();
        });
    });
