...
// Aplicar el middleware de cache y nocache
app.use(cacheHeaders);  // Esto cubre las cabeceras de caché
app.use(nocache());     // Para un control más estricto del no-cache
...
function createBait(id) {
  ...
  let random_y = Math.floor(Math.random() * (CANVAS_HEIGHT - 20)) + 20;
  let random_value = Math.floor(Math.random() * 5) + 1;
  baitNum += 1;
